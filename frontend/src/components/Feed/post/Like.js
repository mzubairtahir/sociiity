...
export default function Like({ id, counter }) {
  const [totalLikes, setTotalLikes] = counter;
  ...
  const likingHandle = async () => {
    ...
    const action = likingState.liked ? "unlike" : "like";
    ...
    try {
      ...
      if (data.status === "success") {
        ...
        setLikingStateWrapper("like", !likingState.liked);
      } else {
      }
    } catch (error) {}
    ...
    <>
      <button
        ...
        className={`${style.interaction} ${
          likingState.liked && style.likedButton
        }`}
      ...
      >
        ...
        <span className={style.interactionNumber}>({totalLikes})</span>
      </button>
    </>
  );
}
